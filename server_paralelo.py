...
def handle_connection(conn):
    try:
        ...
        argumentos = [(linha, B) for linha in subA]

        with Pool(processes=num_workers) as pool:
            resultado = pool.map(multiplicar_linha, argumentos)
...
def run_server(host, port):
    ...
    serv.bind((host, port))
    serv.listen(1)  
    print(f"Servidor TCP (PARALELO) ouvindo em {host}:{port} ... (CTRL+C para encerrar)")
